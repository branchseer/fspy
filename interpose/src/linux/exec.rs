...
use fspy_shared::linux::{ENVNAME_PROGRAM, nul_term::NulTerminated};
use libc::c_char;

use super::{abort::abort_with, client::Client, is_env_reserved};
...
pub fn exec_with_host(
    ...
    let ctx = &mut ExecContext {
        ...
        argv,
        envp,
        cstr_buf: ArrayVec::new(),
        ptr_buf: ArrayVec::new(),
        client,
    };
    Ok(())
}
...
impl<'a> ExecContext<'a> {
    fn copy_cstr(
        &mut self,
        content: impl Iterator<Item = u8>,
    ) -> nix::Result<NulTerminated<'_, u8>> {
        let start = self.cstr_buf.len();
        for byte in content {
            self.cstr_buf
                .try_push(byte)
                .map_err(|_| nix::Error::E2BIG)?;
        }
    ...
    fn prepare_envp(&mut self) -> nix::Result<*const *const c_char> {
        let program_env_ptr = self
            .copy_cstr(
                ENVNAME_PROGRAM
                    .as_bytes()
                    .into_iter()
        ...
        if let Some(envp) = self.envp {
            let envs = envp
                .copied()
                .map(|env_ptr| unsafe { NulTerminated::from_ptr(env_ptr) });
            for env in envs {
                if is_env_reserved(env) {
                    let env_data = env.to_counted().as_slice();
                    abort_with(&[
                        "child process should not spawn with reserved env name (".as_bytes(),
                        env_data,
                        b")",
                    ])
                }
