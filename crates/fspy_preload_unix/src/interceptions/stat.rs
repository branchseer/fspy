...
use crate::{client::{convert::{Fd, PathAt}, global_client}, macros::intercept};
...
unsafe extern "C" fn stat(path: *const c_char, buf: *mut stat_struct) -> c_int {
    unsafe { global_client().handle_open(path, AccessMode::Read); }
...
unsafe extern "C" fn fstat(fd: c_int, buf: *mut stat_struct) -> c_int {
    unsafe { global_client().handle_open(Fd(fd), AccessMode::Read); }
...
unsafe extern "C" fn fstatat(dirfd: c_int, pathname: *const c_char, buf: *mut stat_struct, flags: c_int) -> c_int {
    unsafe { global_client().handle_open(PathAt(dirfd, pathname), AccessMode::Read); }
